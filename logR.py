...
hyperparam_dict = {
    'classification__solver': ['newton-cholesky', 'saga'],
    'classification__C': [0.001, 0.01, 0.1, 1, 10],
    'classification__max_iter': [100, 250, 500, 1000]
}
...
for solver in hyperparam_dict['classification__solver']:
    for ckeys in hyperparam_dict['classification__C']:
        for iter_keys in hyperparam_dict['classification__max_iter']:
            clf = LogisticRegression(solver=solver, C=ckeys, max_iter=iter_keys)
            clf.fit(X_resampled2, y_resampled2)
            y_pred_valid = clf.predict(X_valid)
            score = recall_score(y_valid, y_pred_valid)
            print("Solver = ", solver, "C = ", ckeys, "max_iter = ", iter_keys, "Recall = ", score)
            if score > best_score:
                best_score = score
                best_solver = solver
                best_C = ckeys
                best_max_iter = iter_keys

print("Best solver = ", best_solver, "Best C:", best_C, "Best iter:", best_max_iter)

clf = LogisticRegression(solver=best_solver, C=best_C, max_iter=best_max_iter)

clf.fit(X_resampled, y_resampled)
y_pred_resampled = clf.predict(X_train)
print(classification_report(y_train, y_pred_resampled))
